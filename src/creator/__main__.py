...
# Reads in the stories
        f.ap("""
story_text = manager.read_text(db.nas_root() + "/" + db.read_rand_media_file_of_pool(media_pool_id)[1])
while i <= num_story:
...
while footage_duration_sum < narration.duration():
    ...
    e = AttachMuteVideo(db.nas_root() + "/" + db.read_rand_media_file_of_pool({footage_id})[1], ('center', 'center'))  # get random parkour footage
    e.set_start_and_end(footage_duration_sum, e.duration())  # set times
    e.resize({width}, {height})  # 1920*2=3840 , 1080*2=2160
...
img_location = db.nas_root() + "/" + db.read_specific_media_file_by_id({base_image})[1]
